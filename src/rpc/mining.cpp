...
std::map<std::string, CBlock> mapGPNKAWBlockTemplates;
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated gamepass to.\n"
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when gamepassd is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
// NOTE: Unlike wallet RPC (which use GPN values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "  \"ProofOfGameplayAddress\" : n, (string) Proof-Of-Gameplay Address\n"
            "  \"ProofOfGameplayValue\" : n,   (numeric) Proof-Of-Gameplay Value, 20% of the coinbase\n"
            "  \"DevFundAddress\" : n, (string) DevFund Address\n"
            "  \"DevFundValue\" : n,   (numeric) DevFund Value, 5% of the coinbase\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0 && !gArgs.GetBoolArg("-bypassdownload", false))
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Gamepass is not connected!");

    if (IsInitialBlockDownload() && !gArgs.GetBoolArg("-bypassdownload", false))
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Gamepass is downloading blocks...");
...
//    const struct VBDeploymentInfo& segwit_info = VersionBitsDeploymentInfo[Consensus::DEPLOYMENT_SEGWIT];
    ...
    result.push_back(Pair("DevFundValue", (int64_t)pblock->vtx[0]->vout[2].nValue) );
    ...
    if (pblock->nTime >= nKAWPOWActivationTime) {
        ...
        if (IsValidDestinationString(address)) {
            ...
            if (mapGPNKAWBlockTemplates.count(lastheader)) {
                if (pblock->nTime - 30 < mapGPNKAWBlockTemplates.at(lastheader).nTime) {
            ...
            result.pushKV("pprpcepoch", ethash::get_epoch_number(pblock->nHeight));
            mapGPNKAWBlockTemplates[pblock->GetKAWPOWHeaderHash().GetHex()] = *pblock;
...
static UniValue pprpcsb(const JSONRPCRequest& request) {
    ...
    if (!mapGPNKAWBlockTemplates.count(header_hash))
        throw JSONRPCError(RPC_INVALID_PARAMS, "Block header hash not found in block data");

    std::shared_ptr<CBlock> blockptr = std::make_shared<CBlock>();
    *blockptr = mapGPNKAWBlockTemplates.at(header_hash);
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("estimatefee")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee is deprecated and will be fully removed in v0.17. "
            "To use estimatefee in v0.16, restart gamepassd with -deprecatedrpc=estimatefee.\n"
...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "It is set with the command line argument -gen (or " + std::string(GAMEPASS_CONF_FILENAME) + " setting gen)\n"
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    int numCores = GenerateGamepasss(fGenerate, nGenProcLimit, GetParams());
