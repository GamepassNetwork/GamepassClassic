...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for GPN
    ...
    if (p_rewardSnapshot.strDistributionAsset != "GPN") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is GPN with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using GPN differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "GPN") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Gamepass address (already validated during ownership snapshot creation)
