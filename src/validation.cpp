...
#if defined(NDEBUG)
# error "Gamepass cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Gamepass Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s GPN additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Subsidy is cut by 1% every 43,750 blocks which will occur approximately every 1 month.
    nSubsidy *= std::pow(0.99f,halvings);
    return nSubsidy;
}
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            bool is_spent = inputs.SpendCoin(txin.prevout, &txundo.vprevout.back(), assetCache); /** GPN START */ /* Pass assetCache into function */ /** GPN END */
            assert(is_spent);
        }
    }
    // add outputs
    AddCoins(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** GPN START */ /* Pass assetCache into function */ /** GPN END */
}
...
//                                      << ", " << i << ", " << hash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                Coin coin;
                bool is_spent = view.SpendCoin(out, &coin, &tempCache); /** GPN START */ /* Pass assetsCache into the SpendCoin function */ /** GPN END */
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                int res = ApplyTxInUndo(std::move(undo), view, out, assetsCache); /** GPN START */ /* Pass assetsCache into ApplyTxInUndo function */ /** GPN END */
...
void ThreadScriptCheck() {
    RenameThread("gamepass-scriptch");
...
//                                      << ", " << i << ", " << txhash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
   ...
   /** GPN START */
  	//ProofOfGameplayAddress Assign 20%
  	std::string  GetProofOfGameplayAddress 	= GetParams().ProofOfGameplayAddress();
  	CTxDestination destProofOfGameplay 	   	= DecodeDestination(GetProofOfGameplayAddress);
      if (!IsValidDestination(destProofOfGameplay)) {
  		LogPrintf("IsValidDestination: Invalid Gamepass address %s \n", GetProofOfGameplayAddress);
      }
  	// Parse Gamepass address
	CScript scriptPubKeyProofOfGameplay = GetScriptForDestination(destProofOfGameplay);
	CAmount nProofOfGameplayAmount = GetParams().ProofOfGameplayAmount();
	CAmount nSubsidy = GetBlockSubsidy(pindex->nHeight, chainparams.GetConsensus());
	CAmount nProofOfGameplayAmountValue = nSubsidy*nProofOfGameplayAmount/100;

	//Check 40% Amount
  	if(block.vtx[0]->vout[1].nValue != nProofOfGameplayAmountValue )		{
  		return state.DoS(100,
                           error("ConnectBlock(): coinbase ProofOfGameplay Amount Is Invalid. Actual: %ld Should be:%ld ",block.vtx[0]->vout[1].nValue, nProofOfGameplayAmountValue),
                           REJECT_INVALID, "bad-cb-community-autonomous-amount");
  	}
  	//Check 40% Address
  	if( HexStr(block.vtx[0]->vout[1].scriptPubKey) != HexStr(scriptPubKeyProofOfGameplay) )		{
  		return state.DoS(100,
                           error("ConnectBlock(): coinbase ProofOfGameplay Address Is Invalid. Actual: %s Should Be: %s \n",HexStr(block.vtx[0]->vout[1].scriptPubKey), HexStr(scriptPubKeyProofOfGameplay)),
                           REJECT_INVALID, "bad-cb-fund-address");
  	}
    //DevFundAddress Assign 5%
    std::string  GetDevFundAddress 	= GetParams().DevFundAddress();
    CTxDestination destDevFund 		= DecodeDestination(GetDevFundAddress);
      if (!IsValidDestination(destDevFund)) {
      LogPrintf("IsValidDestination: Invalid Gamepass address %s \n", GetDevFundAddress);
      }
    // Parse Gamepass address
      CScript scriptPubKeyDevFund         	= GetScriptForDestination(destDevFund);
      CAmount nDevFundAmount 		          	= GetParams().DevFundAmount();
      CAmount nDevFundAmountValue	        	= nSubsidy*nDevFundAmount/100;
    //Check 5% Amount
    if(block.vtx[0]->vout[2].nValue != nDevFundAmountValue )		{
      return state.DoS(100,
                           error("ConnectBlock(): coinbase DevFund Amount Is Invalid. Actual: %ld Should be:%ld ",block.vtx[0]->vout[2].nValue, nDevFundAmountValue),
                           REJECT_INVALID, "bad-cb-community-autonomous-amount");
    }
    //Check 5% Address
    if( HexStr(block.vtx[0]->vout[2].scriptPubKey) != HexStr(scriptPubKeyDevFund) )		{
      return state.DoS(100,
                           error("ConnectBlock(): coinbase DevFund Address Is Invalid. Actual: %s Should Be: %s \n",HexStr(block.vtx[0]->vout[2].scriptPubKey), HexStr(scriptPubKeyDevFund)),
                           REJECT_INVALID, "bad-cb-fund-address");
    }
	/** GPN END */
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        if (fDoFullFlush) {
            ...
            if (!CheckDiskSpace((48 * 2 * 2 * pcoinsTip->GetCacheSize()) + assetDirtyCacheSize * 2)) /** GPN START */ /** GPN END */
                return state.Error("out of disk space");
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
...

    {
        ...
        LogPrint(BCLog::BENCH, "  - Flush GPN: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime4 - nTime3) * MILLI, nTimeFlush * MICRO, nTimeFlush * MILLI / nBlocksTotal);
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, &assetCache, true)) /** GPN START */ /*Add asset to function */ /** GPN END*/
