...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** GPN START */
    case OP_GPN_ASSET              : return "OP_GPN_ASSET";
    /** GPN END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_GPN_ASSET) { // OP_GPN_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == GPN_G) { // Check to see if GPN starts at 27 ( this->size() < 105)
                if ((*this)[28] == GPN_P)
                    if ((*this)[29] == GPN_N)
                        index = 30;
            } else {
                if ((*this)[28] == GPN_G) // Check to see if GPN starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == GPN_P)
                        if ((*this)[30] == GPN_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == GPN_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == GPN_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == GPN_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == GPN_G) {
...
{
    // 1 OP_GPN_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_GPN_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_GPN_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_GPN_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_GPN_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
