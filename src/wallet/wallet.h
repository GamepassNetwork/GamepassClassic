...
#ifndef GAMEPASS_WALLET_WALLET_H
#define GAMEPASS_WALLET_WALLET_H
...
public:
     ...
     * on this gamepass node, and set to 0 for transactions that were created
...
public:
    ...
    void AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins,
                            bool fGetGPN = true, bool fOnlyAssets = false,
    ...
    /**
     * Helper function that calls AvailableCoinsAll, used to receive all coins, Assets and GPN
     */
...
#endif // GAMEPASS_WALLET_WALLET_H
