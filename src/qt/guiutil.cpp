...
#include "gamepassaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Gamepass address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new GamepassAddressEntryValidator(parent));
    widget->setCheckValidator(new GamepassAddressCheckValidator(parent));
}
...
bool parseGamepassURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no gamepass: URI
    if(!uri.isValid() || uri.scheme() != QString("gamepass"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!GamepassUnits::parse(GamepassUnits::GPN, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseGamepassURI(QString uri, SendCoinsRecipient *out)
{
    // Convert gamepass:// to gamepass:
    //
    //    Cannot handle this later, because gamepass:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("gamepass://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "gamepass:");
    }
    QUrl uriInstance(uri);
    return parseGamepassURI(uriInstance, out);
}

QString formatGamepassURI(const SendCoinsRecipient &info)
{
    QString ret = QString("gamepass:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(GamepassUnits::format(GamepassUnits::GPN, info.amount, false, GamepassUnits::separatorNever));
        paramCount++;
    }
...
bool openGamepassConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", GAMEPASS_CONF_FILENAME));

    /* Open gamepass.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Gamepass.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Gamepass (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Gamepass (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Gamepass*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "gamepass.desktop";
    return GetAutostartDir() / strprintf("gamepass-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a gamepass.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Gamepass\n";
        else
            optionFile << strprintf("Name=Gamepass (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the gamepass app
...
bool GetStartOnSystemStartup()
{
    CFURLRef gamepassAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (gamepassAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, gamepassAppUrl);

    CFRelease(gamepassAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add gamepass app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, gamepassAppUrl, nullptr, nullptr);
    }
