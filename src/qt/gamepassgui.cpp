...
#include "gamepassgui.h"

#include "gamepassunits.h"
...
const std::string GamepassGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString GamepassGUI::DEFAULT_WALLET = "~Default";

GamepassGUI::GamepassGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void GamepassGUI::loadFonts()
{
	...
	QFontDatabase::addApplicationFont(":/fonts/bgothl");
}


void GamepassGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Gamepass address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and gamepass: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to GPN addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Gamepass addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Gamepass addresses"));
    ...
    openAction->setStatusTip(tr("Open a Gamepass: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Gamepass command-line options").arg(tr(PACKAGE_NAME)));
...
void GamepassGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void GamepassGUI::createToolBars()
{
    if(walletFrame)
    {
        /** GPN START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/gamepasstext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* gamepassLabelLayout = new QVBoxLayout(toolbarWidget);
        gamepassLabelLayout->addWidget(label);
        gamepassLabelLayout->addWidget(toolbar);
        gamepassLabelLayout->setDirection(QBoxLayout::TopToBottom);
        gamepassLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Gamepass Market Price"));
        ...
        QLabel* labelBtcGpn = new QLabel();
        labelBtcGpn->setText("BTC / GPN");
        labelBtcGpn->setContentsMargins(15,0,0,0);
        labelBtcGpn->setFixedHeight(75);
        labelBtcGpn->setAlignment(Qt::AlignVCenter);
        labelBtcGpn->setStyleSheet(STRING_LABEL_COLOR);
        labelBtcGpn->setFont(currentMarketFont);
        ...
        priceLayout->addWidget(labelBtcGpn, 0 , Qt::AlignVCenter | Qt::AlignLeft);
        ...
        // Get the latest Gamepass release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void GamepassGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool GamepassGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool GamepassGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void GamepassGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void GamepassGUI::setWalletActionsEnabled(bool enabled)
{
...
void GamepassGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void GamepassGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void GamepassGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void GamepassGUI::optionsClicked()
{
...
void GamepassGUI::showDebugWindow()
{
...
void GamepassGUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void GamepassGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void GamepassGUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void GamepassGUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void GamepassGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void GamepassGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void GamepassGUI::openClicked()
{
...
void GamepassGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void GamepassGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void GamepassGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void GamepassGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void GamepassGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void GamepassGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** GPN START */
void GamepassGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void GamepassGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void GamepassGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void GamepassGUI::gotoRestrictedAssetsPage()
{
...
void GamepassGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Gamepass network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void GamepassGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void GamepassGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void GamepassGUI::updateHeadersSyncProgressLabel()
{
...
void GamepassGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void GamepassGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Gamepass"); // default title
...
void GamepassGUI::changeEvent(QEvent *e)
{
...
void GamepassGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void GamepassGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void GamepassGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "GPN")
        msg += tr("Amount: %1\n").arg(GamepassUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(GamepassUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void GamepassGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void GamepassGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool GamepassGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool GamepassGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void GamepassGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void GamepassGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void GamepassGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void GamepassGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void GamepassGUI::detectShutdown()
{
...
void GamepassGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void GamepassGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void GamepassGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(GamepassGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(GamepassGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void GamepassGUI::subscribeToCoreSignals()
{
...
void GamepassGUI::unsubscribeFromCoreSignals()
{
...
void GamepassGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void GamepassGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<GamepassUnits::Unit> units = GamepassUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const GamepassUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(GamepassUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(GamepassUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (GamepassUnits::Unit u : GamepassUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(GamepassUnits::name(u)), this);
...
{
    setText(GamepassUnits::name(newUnits));
}
...
void GamepassGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=GPNBTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void GamepassGUI::mnemonic()
{
...
void GamepassGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/GamepassNetwork/GamepassCore/releases"));
