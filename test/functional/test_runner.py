...
`test/functional/test_framework/test_framework.py:GamepassTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/gamepass_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and gamepassd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_gamepassd = config["components"].getboolean("ENABLE_GAMEPASSD")
    if not (enable_wallet and enable_cli and enable_gamepassd):
        print("No functional tests to run. Wallet, utils, and gamepassd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if gamepassd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "gamepassd"]) is not None:
            print("%sWARNING!%s There is already a gamepassd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "GAMEPASSD" not in os.environ:
        os.environ["GAMEPASSD"] = build_dir + '/src/gamepassd' + exeext
        os.environ["GAMEPASSCLI"] = build_dir + '/src/gamepass-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `gamepass-cli help` (`rpc_interface.txt`).
