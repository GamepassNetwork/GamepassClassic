...
The RPM spec file provided here is for gamepass-core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://gamepass.network/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Gamepass git repository](https://github.com/GamepassNetwork/GamepassCore) but are not part of
...
Currently only `Source10` is used, for the example `gamepass.conf` file.
...
in the gamepass git repository. At present only an SVG version of the Gamepass
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the gamepass
...
needed when 0.12.1 is released, a proper fix is already in the Gamepass git
master branch.
...
idea to build Gamepass Core against a static version of that library compiled
...
The `gamepass.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `gamepass-server` package. At this point, most RPM based
...
The `gamepass-server` package should have SELinux support. How to properly do
...
gamepass-core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/GamepassCore/). He has been
packaging Gamepass for Fedora at least since 2012.
...
4. I renamed the `gamepass` package that contains the Qt GUI to `gamepass-core` as
that appears to be how the general population refers to it, in contrast to
`gamepass-xt` or `gamepass-classic`. I wanted to make sure the general population
